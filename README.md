...
<h1>
  <a href="https://welfare.sstxww1.asia/">welfare.sstxww1.asia</a>
</h1>

<h3>公益羊毛站合集 · 白嫖福利中心 · 中转站风险检测 · 投稿与黑榜反馈</h3>
...
[访问主站](https://welfare.sstxww1.asia/) ·
[白嫖福利中心](https://welfare.sstxww1.asia/share) ·
[贡献光荣榜](https://welfare.sstxww1.asia/honor) ·
...
![贡献光荣榜截图](docs/images/honor-board.png)

## 这是什么

羊毛公益中转站是一个面向普通用户的福利导航和风险反馈页面，用来整理公益羊毛站、免费资源入口、半付费资源、逆向专区站点以及需要谨慎访问的黑榜信息。

它不是单纯的链接堆叠页。站点把投稿、举报、联通性检测、贡献 Key、光荣榜、福利分享文章和管理员审核流程放到同一个入口里，让用户能更快找到可用资源，也能把失效、跑路、乱答、疑似提示词投毒、扣费异常等风险反馈回来。

## 这个仓库放什么

本仓库只用于展示网站入口、项目介绍、真实截图、投稿规则、维护说明和反馈入口。
...
| 风险反馈 | 支持举报降智、乱答、疑似提示词投毒、跑路、扣费异常、无法访问等问题。 |
| 专业检测 | 展示协议连通性、模型选择、模型可信度、提示词投毒、SSE 流式稳定性等结果。 |
...
检测系统会先拉取 `/v1/models`，过滤音频、图片、embedding、realtime、codex 等非普通文本模型，再从可用文本模型里选择最适合实测的模型。只有模型真实可请求后，才进入深度探针。

当前检测展示更偏专业口径，但不会乱下结论：协议不兼容、Key 失效、Token 为 0、Chat 全失败这类情况会显示“待复测 / 证据不足”，不会直接标成模型掺假。

| 检测项 | 说明 |
| --- | --- |
| 协议连通性 | 测试 `/v1/models` 和 OpenAI 兼容接口是否可达。 |
| 模型选择 | 从模型列表中选择最佳可用文本模型，而不是盲目使用列表第一个。 |
| 模型可信度 | 通过普通回答、计算、JSON、翻译、短事实题、指令跟随等内容探针判断是否疑似模型掺假。 |
| 提示词投毒 | 使用 canary 安全探针检测隐藏提示词、泄露诱导、危险命令和工具调用注入痕迹。 |
| SSE 流式稳定性 | 检查流式输出是否正常，避免只测非流式导致误判。 |
| Token 与成本 | 记录 prompt/completion/total token，并按官方公开价格估算贡献成本。 |

## 风险状态

| 状态 | 含义 |
| --- | --- |
| 可用 | 基础访问正常，深度检测未发现明显异常。 |
| 模型可能掺假 | 多项内容探针反复异常，疑似模型错配、假模型、降级模型或响应质量异常。 |
| 疑似投毒 | 检测到隐藏提示词、泄密诱导、异常命令或工具调用注入风险。 |
| 高风险 | 多项检测异常，或举报与复核结果显示风险较高。 |
| 待复测 | 缺少有效检测条件、Key 失效、协议不兼容、近期异常待确认，或需要管理员复核。 |
...
4. 如果发现失效、乱答、疑似提示词投毒、跑路或扣费异常，在站点卡片里提交反馈。
5. 想看教程、活动或图文分享，进入 [白嫖福利中心](https://welfare.sstxww1.asia/share)。
6. 想感谢贡献检测 Key 的用户，进入 [贡献光荣榜](https://welfare.sstxww1.asia/honor) 点赞或送花。
...
| 风险提示 | 是否出现乱答、扣费异常、跳转异常、疑似提示词投毒等情况。 |
...
| 风险克制 | 不用一次失败就乱判模型掺假，尽量用多轮证据降低误判。 |
...
公益导航、羊毛网站、免费福利、福利合集、白嫖资源、资源导航、OpenAI 中转站、AI 中转站、逆向专区、风险检测、提示词投毒、模型可信度、SSE 流式稳定性、黑榜、投稿审核、白嫖福利中心、贡献光荣榜。
